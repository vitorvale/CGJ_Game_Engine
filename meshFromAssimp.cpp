...
Assimp::Importer backImporter;
// the global Assimp scene object
const aiScene* scene = NULL;
const aiScene* backScene = NULL;
...
void get_bounding_box_for_node(const aiNode* nd, aiVector3D* min, aiVector3D* max, int index)
{
	aiMatrix4x4 prev;
	unsigned int n = 0, t;
	...
	for (; n < nd->mNumMeshes; ++n) {
		...
		if (index == 0)
			...
			mesh = backScene->mMeshes[nd->mMeshes[n]];
	...
	for (n = 0; n < nd->mNumChildren; ++n) {
		get_bounding_box_for_node(nd->mChildren[n], min, max, index);
	}
}


void get_bounding_box(aiVector3D* min, aiVector3D* max, int index)
{

	min->x = min->y = min->z = 1e10f;
	max->x = max->y = max->z = -1e10f;
	if (index == 0)
		get_bounding_box_for_node(scene->mRootNode, min, max, index);
	else
		get_bounding_box_for_node(backScene->mRootNode, min, max, index);
}
...
bool Import3DFromFile(int index, const std::string& pFile)
{
	...
	if (index == 0) {
		...
		backScene = backImporter.ReadFile(pFile, aiProcessPreset_TargetRealtime_Quality);
		// If the import failed, report it
		if (!backScene) {
			printf("%s\n", backImporter.GetErrorString());
			return false;
		}
	}
	...
	get_bounding_box(&scene_min, &scene_max, index);
	float tmp;
...
bool LoadGLTexturesTUs(const aiScene* scene)  // Create OGL textures objects and maps them to texture units.  
{
	...
	for (int i = 0; itr != textureIdMap.end(); ++i, ++itr)
	{
		...
		Texture2D_Loader(textureIds, filename.c_str(), i, true);  //it already performs glBindTexture(GL_TEXTURE_2D, textureIds[i])
